...
def estimate_watermark(frames):
	...
	gradx = np.array(list(map(lambda x: cv2.Sobel(x, cv2.CV_64F, 1, 0, ksize=KERNEL_SIZE), frames)))
	grady = np.array(list(map(lambda x: cv2.Sobel(x, cv2.CV_64F, 0, 1, ksize=KERNEL_SIZE), frames)))
...
def crop_watermark(gradx, grady, threshold=0.4, boundary_size=2):
	...
	xm, xM = max(0, np.min(x) - boundary_size - 1), np.max(x) + boundary_size + 1
	ym, yM = max(0, np.min(y) - boundary_size - 1), np.max(y) + boundary_size + 1
...
def watermark_detector(img, gx, gy, thresh_low=200, thresh_high=220, printval=False):
	...
	im = img.copy()
	result = cv2.rectangle(im, (y, x), (y+rect[1], x+rect[0]), (255, 0, 0), 5)
	cv2.imshow("image", result)
	cv2.waitKey()
	return (im, (x, y), (rect[0], rect[1]))


def get_cropped_frames(frames, start, end):
	'''
	...
	for i, frame in enumerate(frames):
		...
		frame = frame[_s[0]:(_s[0]+_e[0]), _s[1]:(_s[1]+_e[1]), :]
		# add to list images
